...
# 오류: 초기화 대기를 넣지 않아 아두이노가 데이터를 받지 못함. (발견: 이채민)
...
# 오류: 모델 경로 오타로 파일을 로드하지 못함. (발견: 류이진)
...
# 오류: 서명 이름 오타로 추론이 작동하지 않음. (발견: 한석주)
...
# 오류: 카메라 권한 미설정으로 웹캠이 열리지 않음. (발견: 심진현)
...
while True:
    ...
    # 오류: 정규화 누락으로 잘못된 예측 결과 발생. (발견: 정희재)
    ...
    # 오류: 키 이름 변경 시 누락된 코드 수정 필요. (발견: 이채민)
    ...
    # 오류: np.argmax axis 설정 오류. (발견: 류이진)
...
# 크레딧
# 코드 작성 및 디버깅:
# - 정희재: 정규화 누락 오류 수정 및 주요 코드 작성
# - 김다연: 프레임 캡처 오류 처리 추가
# - 한석주: 아두이노 데이터 전송 문제 해결 및 서명 오류 수정
# - 이채민: 초기화 대기 및 모델 키 이름 수정
# - 심진현: 웹캠 권한 문제 해결
# - 류이진: 모델 경로 수정 및 axis 오류 해결
